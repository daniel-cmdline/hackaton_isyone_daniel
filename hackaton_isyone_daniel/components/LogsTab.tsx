...
export function LogsTab({ logs, tokenAtivo }: LogsTabProps) {
    ...
    <div className="space-y-6 font-mono selection:bg-indigo-500/30">
      ...
      <div className="border-l-2 border-purple-500 pl-4 py-1">
        ...
        <p className="text-xs text-zinc-400 mt-1 max-w-2xl leading-relaxed font-sans">
          Rastreabilidade imutável de eventos armazenados no cluster relacional.
          Toda injeção de payload ou chamada de Isy Token gera uma entrada de
          telemetria assinada.
        </p>
      </div>

      {!tokenAtivo ? (
        <div className="flex flex-col items-center justify-center h-64 bg-zinc-950 rounded-2xl border border-rose-900/40 shadow-inner gap-3 text-zinc-500 text-xs text-center px-6">
          ...
          <span className="text-zinc-500 text-[11px] leading-relaxed max-w-md">
            ...
            para ter acesso aos registros de auditoria e telemetria do sistema.
          </span>
        </div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 select-none">
            ...
            <div className="bg-zinc-950 p-3 rounded-xl border border-zinc-800/60 shadow-inner">
              ...
              <div
                className={`text-xs font-bold mt-1 ${failureCount > 0 ? "text-rose-400 animate-pulse" : "text-zinc-400"}`}
              >
              ...
              <table className="w-full text-left border-collapse">
                <thead className="bg-zinc-950 text-zinc-500 uppercase text-[9px] font-bold tracking-widest border-b border-zinc-900 sticky top-0 z-10 shadow-md select-none">
                  <tr>
                    ...
                    <th className="p-3.5 pr-5 w-48 text-right">
                      TIMESTAMP_LOCAL
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-zinc-900/60">
                  {logs.length === 0 ? (
                    <tr>
                      <td
                        colSpan={4}
                        className="p-8 text-center text-zinc-600 text-xs"
                      >
                        &gt; No transactional inputs found in this node.
                        Pipeline empty.
                      </td>
                    </tr>
                  ) : (
